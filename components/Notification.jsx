...
import { Bell, Check, X, Trophy, UserPlus } from 'lucide-react';
...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    // Query based on user role (GitHub user or Founder)
    const notificationsRef = collection(db, 'notifications');
    const q = isGitHubUser
      ? query(notificationsRef, where('recipientId', '==', user.uid))
      : query(notificationsRef, where('founderId', '==', user.uid));
  ...
  return (
    <div className="max-w-4xl mx-auto pt-24 px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="text-white/70">Stay updated with your application status and new candidates</p>
      </div>
